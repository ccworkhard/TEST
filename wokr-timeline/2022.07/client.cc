...
/*
 * these functions should be compiled only for client before, but now
 * we need compile them because of udas.
 * these functions are declared in libmysql/client_settings.h, but in
 * MYSQL_SERVER mode we should use sql/client_settings.h, these two
 * client_settings.h can be included only one with MYSQL_SERVER macro,
 * so we just extern declare them below.
 */
extern MYSQL_FIELD *cli_list_fields(MYSQL *mysql);
extern bool cli_read_prepare_result(MYSQL *mysql, MYSQL_STMT *stmt);
extern int cli_stmt_execute(MYSQL_STMT *stmt);
extern int cli_unbuffered_fetch(MYSQL *mysql, char **row);
extern const char *cli_read_statistics(MYSQL *mysql);
extern int cli_read_binary_rows(MYSQL_STMT *stmt);
...
/*
 * moved from libmysql.cc because mysql_source.cc need this function.
 */
...
static int native_password_auth_client(MYSQL_PLUGIN_VIO *vio, MYSQL *mysql) {
  ...
  if (mysql->passwd[0]) {
    ...
    char scramble_hex[SCRAMBLE_LENGTH * 2 + 1] = {0};
    char scrambled_hex[SCRAMBLE_LENGTH * 2 + 1] = {0};
    for (int i = 0; i < SCRAMBLE_LENGTH; i++) {
      sprintf(scramble_hex + i * 2, "%.2X", mysql->scramble[i]);
      sprintf(scrambled_hex + i * 2, "%.2X", scrambled[i]);
    }
    scramble_hex[SCRAMBLE_LENGTH * 2] = 0;
    scrambled_hex[SCRAMBLE_LENGTH * 2] = 0;
//  MY_LOG_WARN(0, "JIUREN", K(scramble_hex));
//  MY_LOG_WARN(0, "JIUREN", K(scrambled_hex));
